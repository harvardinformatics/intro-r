...
Welcome to the first part of the [FAS Informatics](https://informatics.fas.harvard.edu/) Intro to R workshop! 
...
[R Terminology](https://informatics.fas.harvard.edu/resources/glossary/#r-terms)
...
For the purpose of demonstrating how to write a data frame to a file, and then load that file back up we can first load data from a file:

> Run the code block below to write the penguins data frame to a csv file:

```{r}
write_csv(penguins,file="mypenguins.csv")
```



> Run the following code block to create an object called `mypenguins` to which the data in *mypenguins.csv* is assigned.

```{r}
mypenguins <- read_csv("mypenguins.csv")
```
...
In tomorrow's workshop, we will teach you how to manipulate data in the [tidyverse](https://www.tidyverse.org/) way, including how to select and filter a data set using certain criteria, how to construct new variables, and how to write the resulting outputs to new files. 
