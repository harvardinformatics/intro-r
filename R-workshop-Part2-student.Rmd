...
Today's data set comes from... wait for it ... penguins! In particular, we are using the [Palmer penguins](https://allisonhorst.github.io/palmerpenguins/){:target="\_blank"} data set contains measurement data for 3 species of penguins collected on three islands at the LTER site in the Palmer Archipelago, Antarctica.
...
glimpse(adelie_males)
print(sum(adelie_males$mass_outlier))
```

> Filter the adelie_males tibble to keep only rows where mass_outlier is TRUE

```{r}
adelie_male_outliers <- filter(adelie_males,mass_outlier==TRUE)
adelie_male_outliers
```
...
print(y)
xy <- left_join(x, y,by="obs_number")
glimpse(xy)
...
adelie <- penguins %>%
                ...
                mutate(normalized_flipper = flipper_length_mm/body_mass_g)
glimpse(adelie)
```
