...
Because the data set we are using today already comes built into the *palmerpenguins* package, we are going to demonstrate the file loading functions by first writing the penguins data.frame objects as a csv file, and then use `read_csv()` to load it. Because `penguins` is a standard data.frame object, we will use the old school `write.csv()` function to create a file, *penguins.csv*, and then load it with the tidyverse `read_csv()` function. We won't assign the loaded csv file to `penguins` because `penguins` already exists as the object that is part of *palmerpenguins*, and we didn't necessarily want remove that object from our environment.
...
```
An important difference between base R loading functions (`read.csv` and `read.table`) and `read_csv` (and `read_table`) is that the tidyverse file loading functions assume the first row of your data contains column names, while the base R functions do not. One of the first things you will notice is that upon loading you will get a brief summary of the tibble contents, such as the tibble dimensions, the column separator, and the names of variables (columns) belonging to each type: *dbl* for numeric and *chr* for character. We can also take a quick peek at the first few rows by simply calling the tibble

**mypenguins is indistinguishable
...
arrange(mypenguins, desc(year),sex)
...
> \*\*Sorting exercise\*. Sort your `mypenguins` tibble by species, then by year, then by body mass, and store the output in a new tibble called `sorted_penguins`.

```{r}

sorted_penguins <- arrange(mypenguins, species, year, body_mass_g)
...
select(mypenguins, !c(island, bill_length_mm, bill_depth_mm))
...
glimpse(select(mypenguins, !c(island, bill_length_mm, bill_depth_mm)))
...
noyear_bill_island <- select(mypenguins, !c(island, bill_length_mm, bill_depth_mm))
...
bill_data <- select(mypenguins, species, sex, bill_length_mm, bill_depth_mm)
...
select(mypenguins, species, sex, starts_with("bill"))
...
adelie <- filter(mypenguins, species=="Adelie")
adelie
...
adelie_males <- filter(mypenguins, species=="Adelie", sex =="male")
adelie_males
...
adelie_gentoo <- filter(mypenguins, species=="Adelie" | species == "Gentoo")
adelie_gentoo
...
chinstrap_females_after2007 <- filter(mypenguins, species=="Chinstrap", sex =="female", year > 2007)
...
filter(mypenguins, species %in% c("Adelie"), (island == "Biscoe" &  sex == "male") | (island == "Torgersen" & sex=="female"))
...
filter(mypenguins, (species == "Adelie" | species == "Gentoo"), (sex == "male" &  year == "2008") | (sex == "female" & year==2009))
...
filter(mypenguins,year< 2009, sex=="male", (species=="Adelie" & body_mass_g > 4043) | (species=="Gentoo" & body_mass_g > 5485)) 
...
> To add a *depth_sizeadj* variable to *mypenguins*, run the code block below:

```{r}

mypenguins <- mutate(mypenguins, depth_sizeadj = bill_depth_mm/bill_length_mm)

```

In redirecting back to the *mypenguins* tibble, we are changing the input data rather than writing a new tibble with this variable added. Note also that this command will **overwrite** the body condition column, if it exists. This is often what is desired, but be aware of this behavior.
...
mypenguins <- mutate(mypenguis, bill_length_mm = na_if(bill_length_mm, -999))
...
mypenguins <- mutate(mypenguins, obs_number=row.names(mypenguins))
...
x <- select(mypenguins, obs_number,body_mass_g)
y <- select(mypenguins, obs_number, species, sex)
xy <- left_join(x,y)
glimpse(xy)
...
mypenguins <- mutate(penguins, id = row_number())
# Then, we use the function pivot_longer() to merge columns together
penguins_long <- pivot_longer(mypenguins, c(bill_length_mm, bill_depth_mm, flipper_length_mm), names_to="morphtype", values_to="mm")
...
new_data <- select(mypenguins, year, species, sex, body_mass_g) %>% filter(species=="Adelie", year == 2008)
...
flipper_norm <- select(mypenguins, species, flipper_length_mm, body_mass_g) %>% filter(species=="Adelie") %>% mutate(norm_flipper_length = flipper_length_mm /body_mass_g)
...
Because the output of the left hand function is piped to the first *unnamed* argument, if you name every argument before data, you'll be okay: `mypenguins %>% filter_command() %>% lm(formula = x ~ y)`
...
summarize(mypenguins, n=n_distinct(species))
...
by_species <- group_by(mypenguins, species)
by_species
...
Now, let's calculate the mean by species for *body_mass_g* and *bill_length_mm*. If you're unsure whether there are NA values in these two columns, you can use the `drop_na()` function to remove rows which contain ANY NA values. To see how you might do this in one command using pipes, we'll start from *mypenguins*.

> Run the following code block:

```{r}

# starting with the mypenguins table
mypenguins %>% 
...
# start with the mypenguins table
mypenguins %>%
